...
class ResNetLayer(torch.nn.Module):
    ...
    def __init__(self, num_input, num_output, num_modules, stride=1, momentum=0.9):
        ...
        ops = [ ResNetModule(num_input, num_output, stride=stride, momentum=momentum), torch.nn.ReLU(inplace=True) ]
...
class ResNet(torch.nn.Module):
    ...
    def __init__(self, num_class, num_input, num_output_base, blocks, stride=1, bn_momentum=0.9):
        """
        ...
        for block_index, num_modules in enumerate(blocks):
            ...
            self._ops.append( ResNetLayer(num_input, num_output, num_modules, stride=block_stride, momentum=bn_momentum) )

            if block_index+2 < len(blocks):
                self._ops.append( torch.nn.ReLU(inplace=True) )
            
            # For the next layer, increase channel count by 2
            num_input  = num_output
            num_output = num_output * 2
            
        self._encoder = torch.nn.Sequential(*self._ops)

        self._classifier = torch.nn.Sequential(torch.nn.ReLU(inplace=True),
            torch.nn.Linear(num_input, num_input),
            torch.nn.ReLU(inplace=True),
            torch.nn.Linear(num_input, num_class)
            )
