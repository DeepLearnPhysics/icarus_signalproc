...
class UResNet(torch.nn.Module):
    ...
    def __init__(self, num_class, num_regress, num_input, num_output_base, blocks, classifier_size=256, bn_momentum=0.9):
        """
        ...
        for i in range(len(features)-1):
            ...
            print('upsampler',i,'...',num_input,'=>',num_output)
            print('decoder',i,'...',num_output*2,'=>',num_output)

        self._classifier, self._regressor = None,None

        if num_class:
            classifier=[]
            classifier.append(torch.nn.Conv2d(num_output_base,classifier_size,kernel_size=1,stride=1,bias=True))
            classifier.append(torch.nn.ReLU(inplace=True))
            classifier.append(torch.nn.Conv2d(classifier_size,classifier_size,kernel_size=1,stride=1,bias=True))
            classifier.append(torch.nn.ReLU(inplace=True))
            classifier.append(torch.nn.Conv2d(classifier_size,num_class,kernel_size=1,stride=1,bias=False))

            self._classifier = torch.nn.Sequential(*classifier)

        if num_regress:
            regressor=[]
            regressor.append(torch.nn.Conv2d(num_output_base,classifier_size,kernel_size=1,stride=1,bias=True))
            regressor.append(torch.nn.ReLU(inplace=True))
            regressor.append(torch.nn.Conv2d(classifier_size,classifier_size,kernel_size=1,stride=1,bias=True))
            regressor.append(torch.nn.ReLU(inplace=True))
            regressor.append(torch.nn.Conv2d(classifier_size,2,kernel_size=1,stride=1,bias=False))
